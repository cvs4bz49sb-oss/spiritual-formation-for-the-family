...
// --- PDF endpoint (serves pre-built PDF) ---
app.get("/api/pdf", (req, res) => {
  if (!isAuthenticated(req)) {
    const isInternal = req.hostname === "localhost" || req.hostname === "127.0.0.1";
    if (!isInternal) return res.status(401).send("Access denied.");
  }
  res.download(path.join(__dirname, "public", "spiritual-formation-for-the-family.pdf"));
});

// --- ePub endpoint (serves pre-built ePub) ---
app.get("/api/epub", (req, res) => {
  if (!isAuthenticated(req)) {
    const isInternal = req.hostname === "localhost" || req.hostname === "127.0.0.1";
    if (!isInternal) return res.status(401).send("Access denied.");
  }
  res.download(path.join(__dirname, "public", "spiritual-formation-for-the-family.epub"));
});
